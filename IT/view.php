...
//exit;
    if($status!='')
    {
         mysql_query("UPDATE ticketmaster SET status=$status,solvedate='$solveddate' WHERE ticketid=$id");
    }
    if($_SESSION['privilege']==2 and $status==2 and $remark!='')
    {
      mysql_query("UPDATE ticketmaster SET status=1,solvedate='$solveddate'  WHERE ticketid=$id");
    }
    if($remark!=''){
 $qry="insert into ticketremarks values('',$id,'$remark','$remakdate','$remarkby','$ipaddress')";
    mysql_query($qry);}
...
<div align=""><span class="Icon thread">Ticket Remarks</span>
    <div id="" align="">
        		   <table width="90%" cellpadding="1" cellspacing="0" border="0">
                     ...
                     <tr class="">
                          <td align =""><p><?php echo $rmk['remark']; ?></p><h4>Remark by: <?php echo $rmk['remarkby']."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;/".$rmk['remakdate']."/&nbsp;&nbsp;&nbsp;".$rmk['ipaddress'];  ?></h4></td>
                     </tr>
			            <?php } ?>
                   </table>
                </div>
