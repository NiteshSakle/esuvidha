...
class Chart
{
	...
	protected function isProjectDB()
	{
		...
		if("civil_ticketmaster" == $this->chrt_array['tables'][0])
			return true;
		if("civil_ticketremarks" == $this->chrt_array['tables'][0])
			return true;
		if("civil_ticketmaster" == $this->chrt_array['tables'][0])
			return true;
		return false;
...
{	
	...
	protected function getCircularGaugeLabel( $seriesNumber, $pointData )
	{
		$label = array(
			...
			"vAlign" => "center",
			"hAlign" => "center",			
			"text" => $this->arrDataSeries[ $seriesNumber ].": ".$pointData["value"]
		);
	...
	{		
		...
		return array( 
			"data" => $pointsData,
			"labelText" => $this->arrDataSeries[ $seriesNumber ].": ".$pointsData[0]["value"]
		);	
	}
}	
